...
from .util.dissim import matching_dissim, ng_dissim, matching_dissim_weights # add matching_dissim_weights
...
class KModes(BaseEstimator, ClusterMixin):
    ...
    """
    Add feature_weight argument in fit function
    by Zhuheng at 1/27/2024
    """
    def fit(self, X, y=None, sample_weight=None, feature_weight=None, **kwargs):
        ...
        """
        add feature weights check here by Zhuheng at 1/27/2024
        """
        ### Start modification here
        if feature_weight is not None:
            print("Feature weights were given!")

        _validate_feature_weight(feature_weight, n_features=X.shape[1],
                                 n_clusters=self.n_clusters)
        if feature_weight is not None:
            self.cat_dissim = matching_dissim_weights
            print("Dissimilarity function considered weights for each categorical variable.")
    ...
    """
    Add feature_weight argument in predict function
    by Zhuheng at 1/27/2024
    """
    def predict(self, X, feature_weight, **kwargs):
        ...
        return labels_cost(X, self._enc_cluster_centroids, self.cat_dissim, feature_weight=feature_weight)[0]
...
"""
Add feature_weight argument in labels_cost function
by Zhuheng at 1/27/2024
"""
def labels_cost(X, centroids, dissim, membship=None, sample_weight=None, feature_weight=None):
    ...
    for ipoint, curpoint in enumerate(X):
        ...
        ### Start modification here
        if feature_weight is not None:
            diss = dissim(centroids, curpoint, weights=feature_weight)
        else:
            diss = dissim(centroids, curpoint, X=X, menbship=membship)
        ### End the modification
...
"""
Add feature_weight argument in k_modes function
by Zhuheng at 1/27/2024
"""
def k_modes(X, n_clusters, max_iter, dissim, init, n_init, verbose, random_state, n_jobs,
            sample_weight=None, feature_weight=None):
    """k-modes algorithm"""
    ...
    if n_jobs == 1:
        for init_no in range(n_init):
            ...
            results.append(_k_modes_single(
                ...
                verbose, seeds[init_no], sample_weight, feature_weight # add feature_weight
            ))
            ### End the modification
    else:
        results = Parallel(n_jobs=n_jobs, verbose=0)(
            ...
            delayed(_k_modes_single)(X, n_clusters, n_points, n_attrs, max_iter,
                                     ...
                                     sample_weight, feature_weight) # add feature_weight
            ### End the modification
...
"""
Add feature_weight argument in _k_modes_signle function
by Zhuheng at 1/27/2024
"""
def _k_modes_single(X, n_clusters, n_points, n_attrs, max_iter, dissim, init, init_no,
                    verbose, random_state, sample_weight=None, feature_weight=None):
    random_state = check_random_state(random_state)
    # _____ INIT _____
    if verbose:
    ...
    for ipoint, curpoint in enumerate(X):
        ...
        ### Start modification here
        if feature_weight is not None:
            clust = np.argmin(dissim(centroids, curpoint, weights=feature_weight))
        else:
    ...
    ### Start modification here
    _, cost = labels_cost(X, centroids, dissim, membship, sample_weight, feature_weight)
    ### End the modification
    ...
    while itr < max_iter and not converged:
        ...
        ### Start modification here
        labels, ncost = labels_cost(X, centroids, dissim, membship, sample_weight, feature_weight)
        ### End the modification
...
"""
Add feature_weight argument in _k_modes_iter function
by Zhuheng at 1/27/2024
"""
...
"""
Add feature weight check function
by Zhuheng at 1/27/2024
"""
### Start modification here
def _validate_feature_weight(feature_weight, n_features, n_clusters):
    if feature_weight is not None:
        if len(feature_weight) != n_features:
            raise ValueError("feature_weight should be of equal size as feature numbers.")
        if any(
            not isinstance(weight, int) and not isinstance(weight, float)
            for weight in feature_weight
        ):
            raise ValueError("feature_weight elements should either be int or floats.")
        if any(weight < 0 for weight in feature_weight):
            raise ValueError("feature_weight elements should be non negative.")
        # if sum([x > 0 for x in feature_weight]) < n_clusters:
        #     raise ValueError("Number of non-zero feature_weight elements should be "
        #                      "larger than the number of clusters.")
### END the modification